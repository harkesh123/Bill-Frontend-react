...
class Bill extends Component {
  ...
  render() {
    if (this.state.lpage !== "false") {
      return (
        <div className="absolute">
          <h3
            onClick={this.onClick}
            className="link tr hover-light-red pointer ttc black pr3"
          >
            product list
          </h3>
      ...
      return (
        <div className="absolute pa5">
            ...
            <table className="f6 w-100 mw7 center" cellSpacing="0">
              <thead>
                <tr className="stripe-dark ">
                  ...
                  <th className="fw6 tl ph5 bg-white" />
                </tr>
              </thead>
              <tbody className="lh-copy">
                {this.state.list.map((d, i) => {
                  return (
                    <tr className="stripe-dark" key={this.state.list._id}>
                      <td className="ph5 pv2">
                        {this.state.list[i].product_code}
                      </td>
                      <td className="ph5 pv2">
                        {this.state.list[i].product_name}
                      </td>
                      <td className="ph5 pv2">
                        {this.state.list[i].product_price}
                      </td>
                      <td className="ph5 pv2">
                        {this.state.list[i].product_gst}
                      </td>
                      <td className="ph5 pv2">
                        <a
                          className="f6 link dim br2 ph1 pv1 mb1 dib white bg-mid-gray"
                          onClick={() => this.onDelete(this.state.list[i])}
                        >
                          delete
                        </a>
                      </td>
          ...
          />
          <a
            className="f6 link dim br2 ph4 pv1 mb1 dib white bg-red"
            onClick={this.onAdd}
          >
            Add
          </a>
